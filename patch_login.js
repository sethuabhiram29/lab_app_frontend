...
// 2. Find the closing tags of the Login/Hero Section
const endGridIndex = code.lastIndexOf('</Grid>');
if (endGridIndex === -1) {
  console.log('Error: Could not find </Grid>');
  process.exit(1);
}

// 3. Insert the new landing page section
const landingPageCode = `
      ...
      {/* ── 4. PROCESS SECTION ───────────────────────────────────────────────── */}
      <Box sx={{ maxWidth: '1000px', margin: '0 auto', px: 4, mb: 16, position: 'relative' }}>
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          <Typography sx={{ 
            fontFamily: '"Playfair Display", "Times New Roman", serif', 
            fontSize: { xs: '2.5rem', md: '3.5rem' }, 
            fontWeight: 700, color: '#30201D', lineHeight: 1.2, mb: 8, maxWidth: '600px'
          }}>
        ...
        <Box sx={{ position: 'relative', ml: { xs: 2, md: 4 } }}>
          ...
          <Box sx={{ position: 'absolute', top: 0, bottom: 0, left: '28px', width: '2px', background: 'linear-gradient(to bottom, rgba(211,28,61,0.2), transparent)', zIndex: 0 }} />

          {[
            { step: '01', title: 'Sample Collection', desc: 'Painless collection at home or at the centre by trained phlebotomists.', icon: <WaterDropIcon sx={{ fontSize: '1.2rem', color: '#D31C3D' }} /> },
            { step: '02', title: 'Precision Analysis', desc: 'Automated analyzers with dual-verification pipelines.', icon: <BiotechIcon sx={{ fontSize: '1.2rem', color: '#D31C3D' }} /> },
            { step: '03', title: 'Clinical Review', desc: 'Every abnormal marker reviewed by senior pathologists.', icon: <TimelineIcon sx={{ fontSize: '1.2rem', color: '#D31C3D' }} /> },
            { step: '04', title: 'Secure Delivery', desc: 'Encrypted reports available instantly inside your portal.', icon: <SecurityIcon sx={{ fontSize: '1.2rem', color: '#D31C3D' }} /> }
          ].map((item, idx) => (
            <motion.div key={idx} initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6, delay: idx * 0.15 }}>
              <Box sx={{ display: 'flex', alignItems: 'flex-start', mb: 6, position: 'relative', zIndex: 1 }}>
                ...
                <Box sx={{ 
                  ...
                  '@media (min-width: 900px)': { right: 'auto', left: '100%', ml: 4, position: 'relative', transform: 'none', top: 'auto' }
                }}>
                  {item.icon}
                </Box>
...
const updatedCode = code.substring(0, endGridIndex + 7) + landingPageCode + code.substring(code.indexOf('  );', endGridIndex));

fs.writeFileSync(filePath, updatedCode);
console.log('Done Login.js patch!');
